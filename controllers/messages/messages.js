...
router.post("/", authorizeAndExtractToken, async (req, resp, next) => {
    ...
    toList = [{ name: "irina.toma" }, { name: "crawler", group: true }] // TODO: delete
    ...
    for (let user of toList) {
        if (user.group) {
            ...
            if (!mailingEntryList.length == 0) {
                ...
                let msgReceiver = new Message(null, currentId, currentId, params.body, params.title, null, mailingListName);
                ...
                for (let entry of mailingEntryList) {
                    ...
                    let msgSender = new Message(to, currentId, to, params.body, params.title, null, null);
